...
export default function EmployeeLayout() {
  ...
  const navItems = [
    { name: "Dashboard Home", path: "/employee", icon: <LayoutDashboard size={18} /> },
    { name: "Active Goal Sheet", path: "/employee/goals", icon: <Target size={18} /> },
    { name: "Achievement Update", path: "/employee/achievements", icon: <Trophy size={18} /> },
    { name: "Notifications", path: "/employee/notifications", icon: <Bell size={18} /> },
    { name: "Settings / Profile", path: "/employee/settings", icon: <UserCircle size={18} /> },
  ];
    ...
    <div className="flex h-screen bg-[#F9FAFB] overflow-hidden">
      ...
      `}>
        ...
        <nav className="flex-1 px-4 space-y-1">
          <p className="px-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-4">Employee Portal</p>
          {navItems.map((item) => {
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 lg:px-8 shrink-0">
          ...
          <div className="flex items-center gap-3">
             {/* Simple notification dot indicator for employee */}
